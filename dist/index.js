...
async function fetchText(url, accept) {
    ...
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Network error fetching ${url}: ${String(networkErr)}`);
    }
    const body = await response.text();
    if (!response.ok) {
        throw new McpError(ErrorCode.InternalError, `HTTP ${response.status} ${response.statusText} from ${url}\nResponse body: ${body}`);
    }
    return body;
}
/** Parse XML-ish response to extract codelist entries as { id, name }[]. */
function parseCodelist(xml) {
    const items = [];
    const codeRegex = /<(str|structure):Code\s+id="([^"]+)"[^>]*>([\s\S]*?)<\/(str|structure):Code>/g;
...
function resolveDatasetType(datasetType) {
    const t = datasetType.trim().toUpperCase();
    if (t === "SDG")
        return "SDG";
    if (t === "THEME" || t === "THEMATIC")
        return "Theme";
    throw new McpError(ErrorCode.InvalidParams, `Invalid datasetType "${datasetType}". Must be "SDG" or "Thematic".`);
}
// ---------------------------------------------------------------------------
// Tool schemas (Zod)
// ---------------------------------------------------------------------------
...
async function getIndicators() {
    ...
    lines.push("");
    lines.push("Browse the list below. The Indicator Code is the segment after the last # in the 'val' field.");
    lines.push("Note whether the indicator belongs to 'SDG' or 'Thematic' datasets.");
    lines.push("");
...
async function getIndicatorMetadata(indicatorCode, datasetType) {
    const dsType = resolveDatasetType(datasetType);
    const url = `${REST_API_BASE}/v2/data/dataflow/ESCAP/${dsType}_Dataflow/2.10/` +
    ...
    try {
        ...
        return (`Metadata for indicator: ${indicatorCode} (${dsType} dataset)\n` +
            "=".repeat(50) +
            "\n" +
...
async function checkDataAvailability(indicatorCode, datasetType, countryId, startYear, endYear) {
    const dsType = resolveDatasetType(datasetType);
    const url = `${REST_API_BASE}/availableconstraint/ESCAP,${dsType}_Dataflow,2.10/` +
        `.${indicatorCode}..A` +
        `?startPeriod=${startYear}&endPeriod=${endYear}` +
        `&dimensionAtObservation=AllDimensions&mode=available`;
    const raw = await fetchText(url, "application/vnd.sdmx.structure+xml");
    const refAreaMatch = raw.match(/id="REF_AREA"[^>]*>([\s\S]*?)<\/[\w:]*KeyValue>/);
    ...
    if (refAreaMatch) {
        ...
        const allValues = [
            ...raw.matchAll(/<(?:\w+:)?Value[^>]*>([A-Z0-9_]+)<\/(?:\w+:)?Value>/g),
        ];
...
async function getData(indicatorCode, datasetType, countryId, startYear, endYear) {
    const dsType = resolveDatasetType(datasetType);
    const country = countryId?.trim() ?? "";
    const url = `${REST_API_BASE}/data/ESCAP,${dsType}_Dataflow,2.10/` +
        `${country}.${indicatorCode}..A` +
        `?startPeriod=${startYear}&endPeriod=${endYear}` +
        `&dimensionAtObservation=AllDimensions`;
    const raw = await fetchText(url);
    const header = `Data for indicator: ${indicatorCode} (${dsType})\n` +
...
function createServer() {
    const server = new Server({ name: "escap-data-explorer", version: "1.0.0" }, { capabilities: { tools: {} } });
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: "get_indicators",
                description: "Step 1: Retrieve the full hierarchy of available SDG and Thematic indicators from UNESCAP Data Explorer. " +
            ...
            },
            {
                ...
                description: "Step 2: Retrieve metadata (dimensions, attributes, structure) for a specific indicator. " +
            ...
            {
                name: "get_countries",
                description: "Step 3: Retrieve the full list of countries and reference areas available in UNESCAP Data Explorer, " +
            ...
            },
            {
                ...
                description: "Step 4: Check whether data is available for a given indicator, country, and time period. " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        countryId: {
                            type: "string",
                            description: "Country/ref_area code from get_countries (optional — omit for all countries)",
                        },
                        startYear: {
                            type: "number",
            ...
            {
                name: "get_data",
                description: "Step 5: Retrieve actual data for a given indicator, optional country, and time period. " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        countryId: {
                            type: "string",
                            description: "Country/ref_area code (optional — omit for all countries)",
                        },
        ...
        try {
            ...
            switch (name) {
                ...
                case "check_data_availability": {
                    ...
                    result = await checkDataAvailability(p.indicatorCode, p.datasetType, p.countryId, p.startYear, p.endYear);
                    break;
                }
                case "get_data": {
                    const p = GetDataSchema.parse(args);
                    result = await getData(p.indicatorCode, p.datasetType, p.countryId, p.startYear, p.endYear);
                    break;
                }
                default:
            ...
            if (err instanceof z.ZodError) {
                throw new McpError(ErrorCode.InvalidParams, `Invalid parameters: ${err.errors
    ...
    const httpServer = http.createServer(async (req, res) => {
        ...
        if (req.method === "POST" && req.url === "/message") {
            ...
            if (!sessionId || !transports.has(sessionId)) {
                ...
                res.end(JSON.stringify({ error: "Missing or unknown x-session-id header" }));
                return;
            }
